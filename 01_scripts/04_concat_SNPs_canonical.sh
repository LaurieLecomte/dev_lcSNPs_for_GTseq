...
zless $SNP_DIR/all/all_maf"$MIN_MAF"_pctind"$PERCENT_IND"_maxdepth"$MAX_DEPTH_FACTOR"_chr"$FIRST_CHR"_canon.beagle.gz | head -n1 > $SNP_DIR/all/all_maf"$MIN_MAF"_pctind"$PERCENT_IND"_maxdepth"$MAX_DEPTH_FACTOR"_all_chrs_canon.beagle

# 2. Append beagle’s contents for all chromosomes
less $CHR_LIST | while read CHR
do
  # extract the right beagle file for a given chr 
	BEAGLE_FILE=$(ls -1 $SNP_DIR/all/all_maf"$MIN_MAF"_pctind"$PERCENT_IND"_maxdepth"$MAX_DEPTH_FACTOR"_chr*_canon.beagle.gz | grep $CHR) # extract the right beagle file for a given chr 
 echo "appending file $BEAGLE_FILE"
	# Extract all lines except first one and append to ALL_CHR.beagle
  zless $BEAGLE_FILE  | grep -v ^marker >> $SNP_DIR/all/all_maf"$MIN_MAF"_pctind"$PERCENT_IND"_maxdepth"$MAX_DEPTH_FACTOR"_all_chrs_canon.beagle
done

# 3. Compress
bgzip -f $SNP_DIR/all/all_maf"$MIN_MAF"_pctind"$PERCENT_IND"_maxdepth"$MAX_DEPTH_FACTOR"_all_chrs_canon.beagle
...
zless $SNP_DIR/all/all_maf"$MIN_MAF"_pctind"$PERCENT_IND"_maxdepth"$MAX_DEPTH_FACTOR"_chr"$FIRST_CHR"_canon.mafs.gz | head -n1 > $SNP_DIR/all/all_maf"$MIN_MAF"_pctind"$PERCENT_IND"_maxdepth"$MAX_DEPTH_FACTOR"_all_chrs_canon.mafs

# 2. Append beagles contents for all chromosomes
less $CHR_LIST | while read CHR
do
  # extract the right beagle file for a given chr 
	MAFS_FILE=$(ls -1 $SNP_DIR/all/all_maf"$MIN_MAF"_pctind"$PERCENT_IND"_maxdepth"$MAX_DEPTH_FACTOR"_chr*_canon.mafs.gz | grep $CHR) # extract the right beagle file for a given chr 
	echo "appending file $MAFS_FILE"
  # Extract all lines except first one and append to ALL_CHR.mafs
  zless $MAFS_FILE  | grep -v ^chromo >> $SNP_DIR/all/all_maf"$MIN_MAF"_pctind"$PERCENT_IND"_maxdepth"$MAX_DEPTH_FACTOR"_all_chrs_canon.mafs
done


# Prepare lists of sites for next analyses
...
awk '{print $1"\t"$2-1"\t"$2}' 02_infos/sites_all_maf"$MIN_MAF"_pctind"$PERCENT_IND"_maxdepth"$MAX_DEPTH_FACTOR"_canonical_sites > 02_infos/sites_all_maf"$MIN_MAF"_pctind"$PERCENT_IND"_maxdepth"$MAX_DEPTH_FACTOR"_canonical_sites.bed

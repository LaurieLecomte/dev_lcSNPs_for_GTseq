...
do 
  ...
  Rscript 01_scripts/utils/plot_admix.R $VALID_DIR/"$(basename -s '.beagle.gz' $INPUT)"_K"$i".qopt $ID_POP
done
...
do
  ...
  do 
    ...
    Rscript 01_scripts/utils/plot_admix.R $VALID_DIR/subsets/"$(basename -s '.beagle.gz' $file)"_K"$i".qopt $ID_POP
  done
